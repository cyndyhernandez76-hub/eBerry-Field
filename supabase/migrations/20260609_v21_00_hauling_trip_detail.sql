...
-- Assumes fleet.trip_event(trip_event_id, tenant_id, trip_id, event_type, notes, occurred_at)
-- already exists (fuel/maintenance events use it). Created here if missing for safety.

create table if not exists fleet.trip_event (
...
create index if not exists trip_event_trip_idx on fleet.trip_event (trip_id, occurred_at);
...
as $$
  select t.trip_id,
         ...
         t.origin_name, t.origin_address, t.dest_name, t.dest_address,
         t.planned_miles, t.actual_miles, t.actual_start, t.actual_end,
         t.bill_to_customer_id,
         coalesce(c.customer_name, t.bill_to_name) as bill_to_name,
         coalesce(t.is_related_party_use, false) as is_related_party_use,
         coalesce((
           select jsonb_agg(jsonb_build_object(
             'event_type', e.event_type, 'notes', e.notes, 'occurred_at', e.occurred_at
           ) order by e.occurred_at)
           from fleet.trip_event e where e.trip_id = t.trip_id
         ), '[]'::jsonb) as events
    from fleet.trip t
    left join billing.customer c on c.customer_id = t.bill_to_customer_id
...
declare v_id uuid;
begin
  insert into fleet.trip_event (tenant_id, trip_id, event_type, notes, occurred_at)
  values (p_tenant_id, p_trip_id, p_event_type, p_notes, now())
  returning trip_event_id into v_id;

  -- Keep trip.status coherent with lifecycle events.
  if p_event_type = 'paused' then
    update fleet.trip set status = 'paused' where trip_id = p_trip_id and tenant_id = p_tenant_id;
  elsif p_event_type = 'resumed' then
    update fleet.trip set status = 'in_progress' where trip_id = p_trip_id and tenant_id = p_tenant_id;
  end if;
...
-- ── Start (or resume) a hauling trip for a driver and return its id ──
-- If the driver already has a non-completed trip, return that instead of creating
-- a duplicate, so re-tapping "Hauler" lands on the same in-progress trip.
...
begin
  select trip_id into v_trip
   ...
   where tenant_id = p_tenant_id
     ...
     and coalesce(status,'scheduled') not in ('completed','cancelled')
  ...
  insert into fleet.trip (tenant_id, driver_id, vehicle_id, status, trip_code, created_at)
  values (p_tenant_id, p_driver_id, p_vehicle_id, 'in_progress',
